...
sudo apt install -y \
    ...
    libusb-dev zlib1g-dev libftdi-dev libjpeg-dev libkrb5-dev libnova-dev \
    libtiff-dev libfftw3-dev librtlsdr-dev libcfitsio-dev libgphoto2-dev \
    libusb-1.0-0-dev libdc1394-dev libboost-regex-dev libcurl4-gnutls-dev libtheora-dev 

sudo apt-get -y install \
    libnova-dev libcfitsio-dev libusb-1.0-0-dev zlib1g-dev libgsl-dev \
    libjpeg-dev libcurl4-gnutls-dev libtiff-dev \
    libfftw3-dev libftdi-dev libgps-dev libraw-dev libdc1394-dev \
    libgphoto2-dev libboost-dev libboost-regex-dev librtlsdr-dev \
    liblimesuite-dev libftdi1-dev libavcodec-dev libavdevice-dev \
...
# Deactivate pifinder service during build phase. (Memory is low and else build may fail.)
...
if [ -d "./indi/.git" ]; then
    echo "Directory ~/indi already exists and is a Git repository. Skipping clone."
else
    echo "Cloning INDI repository..."
    rm -rf ./indi 2>/dev/null || true
...
if [ -d "./indi-3rdparty/.git" ]; then
    echo "Directory ~/indi-3rdparty exists and is a Git repository. Skipping clone."
else
    echo "Cloning INDI 3rd party repository..."
    rm -rf ./indi-3rdparty 2>/dev/null || true
    git clone --branch v2.1.6.2 --depth 1 https://github.com/indilib/indi-3rdparty.git
fi
...
sudo apt install -y \
    swig libdbus-1-3 libdbus-1-dev libglib2.0-0 libglib2.0-bin \
    libglib2.0-dev python-setuptools python-dev-is-python3
...
# Setup indiwebmanager as a systemd service
CURRENT_USER=$(whoami)
...
echo ""
echo "Set timezone via: sudo raspi-config"
