...
system clock 또는 RTC 쓰기를 실제로 수행하려면 선택 helper 서비스를 설치하고 활성화해야 합니다. 실외 최종 테스트 전에는 dry-run 모드로 먼저 검증하는 것을 권장합니다.
...
실제 system clock/RTC 쓰기를 허용하려면 dry-run이 아닌 일반 모드로 전환합니다.
...
`pifinder_gps_time_sync.service` helper는 root로 실행되며 요청을 검증한 뒤에만 `/usr/bin/date -u --set @<timestamp>` 또는 `/usr/sbin/hwclock --utc --set --date <utc-time>`을 실행합니다. dry-run 모드에서는 같은 검증을 수행하지만 실제 명령은 실행하지 않고 `dry run: ...` 결과만 기록합니다. 요청은 같은 부팅 세션의 최신 요청인지, GPS 상태가 `stable`인지, 마지막 샘플이 `valid: true`인지 확인됩니다.
...
4. system clock 또는 RTC 동기화를 테스트할 때만 helper를 `enable-dry-run`으로 활성화하고 `gps_time_sync_system_clock` 또는 `rtc_sync`를 켭니다.
...
7. dry-run 결과가 정상일 때만 `./scripts/install_gps_time_sync_helper.sh enable`로 실제 쓰기 모드로 전환합니다.
