...
class IndiEventRecorder(PyIndi.BaseClient):
    ...
    def _extract_property_data(self, prop) -> Dict[str, Any]:
        ...
        try:
            if prop.getType() == PyIndi.INDI_TEXT:
                ...
                for widget in text_prop:
                    prop_data["widgets"].append({
                        ...
                        "label": getattr(widget, 'getLabel', lambda: getattr(widget, 'getName', lambda: 'Unknown')())(),
                ...
                for widget in switch_prop:
                    prop_data["widgets"].append({
                        "name": getattr(widget, 'getName', lambda: 'Unknown')(),
                        "label": getattr(widget, 'getLabel', lambda: getattr(widget, 'getName', lambda: 'Unknown')())(),
                        "state": getattr(widget, 'getStateAsString', lambda: 'Unknown')()
                    })
            ...
            self.logger.warning(f"Failed to extract widget data for property {prop.getName()}: {e}")
            # Add minimal widget info if extraction fails
            prop_data["widgets"] = [{"name": "unknown", "label": "Failed to extract", "value": "error"}]

        return prop_data

    def newDevice(self, device):
        """Called when a new device is detected."""
        self._write_event("new_device", {
    ...
    def removeDevice(self, device):
        ...
        self._write_event("remove_device", {
            "device_name": device.getDeviceName()
        })
    ...
    def updateProperty(self, prop):
        ...
        self.logger.debug(f"Property updated: {prop.getName()} on {prop.getDeviceName()}")

    def removeProperty(self, prop):
        """Called when a property is deleted."""
        self._write_event("remove_property", {
            "name": prop.getName(),
            "device_name": prop.getDeviceName(),
            "type": prop.getTypeAsString()
        })
        self.logger.info(f"Property removed: {prop.getName()} on {prop.getDeviceName()}")

    def newMessage(self, device, message):
        """Called when a new message arrives from a device."""
        self._write_event("new_message", {
            "device_name": device.getDeviceName(),
            "message": message
        })
    ...
    def serverConnected(self):
        ...
        self._write_event("server_connected", {
            "host": self.getHost(),
            "port": self.getPort()
        })
    ...
    def serverDisconnected(self, code):
        ...
        self._write_event("server_disconnected", {
            "host": self.getHost(),
            "port": self.getPort(),
            "exit_code": code
        })
    ...
    def close(self):
        ...
        if hasattr(self, 'file_handle'):
            ...
            self.logger.info(f"Recorded {self.event_count} events to {self.output_file}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Record INDI server events to JSON file')
    ...
    parser.add_argument('--output', default='indi_events.jsonl', help='Output file for events (optional, default: indi_events.jsonl)')
