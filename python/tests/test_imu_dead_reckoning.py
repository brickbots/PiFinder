...
class TestPredict:
    ...
    def test_predict_rotates_by_imu_delta(self, dr):
        ...
        q_eq2pointing_at_solve = (dr.q_eq2x * q0 * dr.q_imu2cam).normalized()

        delta = 0.05
        q1 = (q0 * qt.axis_angle2quat([0, 0, 1], delta)).normalized()
        out = dr.predict(q1)
        q_eq2pointing_pred = (dr.q_eq2x * q1 * dr.q_imu2cam).normalized()
        moved = qt.get_quat_angular_diff(q_eq2pointing_at_solve, q_eq2pointing_pred)
