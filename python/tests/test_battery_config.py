...
@pytest.mark.unit
def test_plan_from_defaults_writes_all_three():
    """From the factory/post-reset defaults, all three registers change.

    REG07 (watchdog) must come first so disabling it precedes the
    current-limit writes.
    """
    writes = plan_charging_writes(DEFAULT_REG00, DEFAULT_REG04, DEFAULT_REG07)

    assert [reg for reg, _ in writes] == [REG07, REG00, REG04]
    values = dict(writes)
...
def test_plan_preserves_en_pumpx():
    ...
    writes = dict(plan_charging_writes(DEFAULT_REG00, reg04, DEFAULT_REG07))
...
def test_plan_is_idempotent_when_already_configured():
    ...
    configured = plan_charging_writes(DEFAULT_REG00, DEFAULT_REG04, DEFAULT_REG07)
    reg00, reg04, reg07 = DEFAULT_REG00, DEFAULT_REG04, DEFAULT_REG07
    ...
    assert plan_charging_writes(reg00, reg04, reg07) == []
...
def test_plan_reasserts_only_drifted_register():
    ...
    writes = plan_charging_writes(reg00, reg04, reg07)
