...
# NM keyfile helpers — SSID becomes a DECIMAL byte list (the only list form
# NM's keyfile parser accepts; hex is silently kept as a literal-string SSID,
# mangling the network name so the device can never join it), PSK gets minimal
# keyfile escaping (backslash + semicolon), filename is sanitized so a hostile
# or unusual SSID can't escape NM_DIR.
ssid_to_bytelist() {
    printf '%s' "$1" | od -An -tu1 | tr -s ' \n' ' ' | sed 's/^ //; s/ $//; s/ /;/g; s/$/;/'
}
escape_kf() {
        ...
        case "${line}" in
            ...
            "}")
                if [ "${IN_NET}" = "1" ] && [ -n "${SSID}" ]; then
                    SSID_BYTES=$(ssid_to_bytelist "${SSID}")
...
NMEOF
            ...
            ssid=*)
                if [ "${IN_NET}" = "1" ]; then
                    VAL=$(printf '%s' "${line}" | sed 's/^ssid=//')
                    case "${VAL}" in
                        \"*\")
                            SSID=$(printf '%s' "${VAL}" | sed 's/^"//; s/"$//')
                            ;;
                        *)
                            # Unquoted wpa_supplicant ssid values are hex-encoded
                            # (how SSIDs with special characters are stored) —
                            # decode, or the network name is the hex garbage.
                            if printf '%s' "${VAL}" | grep -qE '^([0-9a-fA-F][0-9a-fA-F])+$'; then
                                SSID=$(printf "$(printf '%s' "${VAL}" | sed 's/\(..\)/\\x\1/g')")
                            else
                                SSID="${VAL}"
                            fi
                            ;;
