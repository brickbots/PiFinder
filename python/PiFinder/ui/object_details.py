...
class UIObjectDetails(UIModule):
    ...
    def serialize_ui_state(self) -> dict:
        ...
        try:
            ...
            if self.object:
                object_info = {
                    "display_name": getattr(self.object, 'display_name', str(self.object)),
            ...
            try:
                if hasattr(self, 'observations_db') and self.object:
                    observation_count = self.observations_db.get_observation_count(
                        self.object.catalog, self.object.sequence
                    ) if hasattr(self.object, 'catalog') and hasattr(self.object, 'sequence') else 0
            except Exception:
                observation_count = 0
