...
class UIRADecEntry(UIModule):
    def __init__(self, *args, **kwargs):
        ...
        # Layout constants - use more space
    ...
    def get_field_positions(self):
        ...
        if self.coord_format == 0:  # HMS/DMS - 6 fields
            # RA fields with generous spacing
            ra_start_x = 10
            positions.append((ra_start_x, self.ra_y, self.small_field_width))  # RA_H
            positions.append((ra_start_x + 28, self.ra_y, self.small_field_width))  # RA_M  
            positions.append((ra_start_x + 56, self.ra_y, self.small_field_width))  # RA_S
            # DEC fields with generous spacing
            dec_start_x = 10
            positions.append((dec_start_x, self.dec_y, self.small_field_width))  # DEC_D
            positions.append((dec_start_x + 28, self.dec_y, self.small_field_width))  # DEC_M
            positions.append((dec_start_x + 56, self.dec_y, self.small_field_width))  # DEC_S
        else:  # Mixed or Decimal - 2 fields
            positions.append((10, self.ra_y, 50))  # RA field - wider
            positions.append((10, self.dec_y, 50))  # DEC field - wider
        return positions
    ...
    def draw_coordinate_fields(self):
        ...
        if self.coord_format == 0:
            # Draw colons for RA with better positioning
            self.draw.text((34, self.ra_y + 5), ":", font=self.base.font, fill=self.red)
            self.draw.text((62, self.ra_y + 5), ":", font=self.base.font, fill=self.red)
            # Draw colons for DEC with better positioning
            self.draw.text((34, self.dec_y + 5), ":", font=self.base.font, fill=self.red)
            self.draw.text((62, self.dec_y + 5), ":", font=self.base.font, fill=self.red)
            
        # Draw format indicators
        if self.coord_format == 1:
            self.draw.text((65, self.ra_y + 5), "h", font=self.base.font, fill=self.half_red)
            self.draw.text((65, self.dec_y + 5), "°", font=self.base.font, fill=self.half_red)
        elif self.coord_format == 2:
            self.draw.text((65, self.ra_y + 5), "°", font=self.base.font, fill=self.half_red)
            self.draw.text((65, self.dec_y + 5), "°", font=self.base.font, fill=self.half_red)
    ...
    def draw_bottom_bar(self):
        ...
        # Format switch instruction
        format_text = f"󰝤 {self.format_names[self.coord_format]}"
        self.draw.text((5, bar_y + 3), format_text, font=self.base.font, fill=self.red)
        
        # Navigation instructions - new layout
        nav_text = "↕ Navigate"
