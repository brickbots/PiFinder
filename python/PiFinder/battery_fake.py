...
def battery_monitor(shared_state, console_queue, ui_queue, log_queue):
    ...
    while True:
        ...
        # BRANCH-ONLY: log every poll like the real monitor; blind polls
        # log blank cells (the fake's `voltage` still holds the last sane
        # value then, so feed from the state, not the counter).
