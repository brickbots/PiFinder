...
class aiogps(gps):  # pylint: disable=R0902
    ...
    def __init__(
        ...
        self.connection_args = connection_args or {"host": self.host, "port": self.port}
    ...
    async def read(self) -> Union[dictwrapper, str]:
        ...
        while True:
            ...
            try:
                ...
                self.bresponse = await asyncio.wait_for(
                    reader, rx_timeout, loop=self.loop
                )
                self.response = polystr(self.bresponse)
    ...
    async def stream(
        self, flags: Optional[int] = 0, devpath: Optional[str] = None
    ) -> None:
