    ...
    while True:
        ...
        try:
            ...
            if telemetry is not None:
                # BRANCH-ONLY: log the row before the trigger below so the
                # poll that fires the shutdown is still on disk. Blind
                # polls have no voltage to feed the curve — log a blank
                # (the analysis tool reads blank/None as ADC-blind).
                telemetry.log(
                    state,
                    None
                    if state.battery_voltage is None
                    else estimate_soc(state.battery_voltage),
                )
