"""
LEGACY copy of the pre-refactor ImuDeadReckoning, preserved so the new
simplified class in imu_dead_reckoning.py can be cross-checked against it.

This file is temporary. Delete it (and
tests/test_imu_dead_reckoning_equivalence.py) once the new class is
...
class ImuDeadReckoningLegacy:
    ...
    def update_imu(self, q_x2imu: quaternion.quaternion):
        ...
        if not np.isnan(self.q_eq2x):
            ...
            self.q_eq2cam = (self.q_eq2x * q_x2imu * self.q_imu2cam).normalized()
            self.q_eq2scope = (self.q_eq2cam * self.q_cam2scope).normalized()
    ...
    def reset(self):
        """
        Resets the internal states so that q_eq2x will be re-estimated.
        """
        self.q_eq2x = None
        self.tracking = False
    ...
    @staticmethod
    def _get_q_imu2cam(screen_direction: str) -> quaternion.quaternion:
        """
        ...
        if screen_direction == "left":
            ...
            q2 = qt.axis_angle2quat([0, 0, 1], np.pi)
            q_imu2cam = (q1 * q2).normalized()
        elif screen_direction == "rev4_left":
            ...
            q2 = qt.axis_angle2quat([0, 0, 1], -np.pi / 2)
            q_imu2cam = (q1 * q2).normalized()
        elif screen_direction == "rev4_right":
            ...
            q2 = qt.axis_angle2quat([0, 0, 1], -np.pi / 2)
            q_imu2cam = (q1 * q2).normalized()
        elif screen_direction == "rev4_straight":
