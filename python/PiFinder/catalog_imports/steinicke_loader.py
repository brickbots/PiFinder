...
def _check_required_files() -> bool:
    ...
    descriptions_json = utils.astro_data_dir / "ngc_ic_m/steinicke/ngc2000_descriptions.json"
...
def _process_ngc2000_descriptions():
    ...
    try:
        ...
        descriptions_json = steinicke_dir / "ngc2000_descriptions.json"
        ngc_count, ic_count = process_ngc2000_to_json(str(ngc2000_file), str(descriptions_json))
        logging.info(f"Processed {ngc_count} NGC and {ic_count} IC descriptions, saved to {descriptions_json}")
...
def load_ngc_catalog():
    ...
    # Load descriptions
    with open(utils.astro_data_dir / 'ngc_ic_m/steinicke/ngc2000_descriptions.json', 'r') as f:
        catalog_data = json.load(f)
    ngc_dict, ic_dict = {int(k): v for k, v in catalog_data['ngc'].items()}, {int(k): v for k, v in catalog_data['ic'].items()}
    ...
    for key, objects in object_groups.items():
        ...
        if get_priority(best_object) < 999:
            ...
            if len(objects) > 1:
                ...
                if skipped:
                    logging.debug(f"Selected {prefix}{number}{best_object.get('extension_letter') or best_object.get('component') or ''}, "
    ...
    for obj in tqdm(selected_objects, desc="Preparing objects", leave=False):
        ...
        if obj.get('diameter_larger'):
            size = f"{obj['diameter_larger']}"
            if obj.get('diameter_smaller'):
                size += f"x{obj['diameter_smaller']}"
    ...
    try:
        for obj in tqdm(prepared_objects, desc="Inserting objects", leave=False):
            obj.insert()
            object_id_desc_dict[obj.object_id] = obj.description
        conn.commit()  # Single commit for all operations
    finally:
        objects_db.bulk_mode = False
        NewCatalogObject.clear_shared_finder()

    # Second pass: Process Messier objects and common names
    logging.info("Processing Messier objects and common names...")
    seen = set()

    for obj in tqdm(selected_objects, leave=False):
        prefix = obj['catalogue_prefix']  # Already validated in earlier filter
        sequence = obj['catalogue_number']
        catalog = "NGC" if prefix == 'N' else "IC"

        object_id = f"{catalog}{sequence}"
        remarks = obj.get('remarks', [])
        remarks = remarks if remarks is not None else []

        # Look for Messier designations in remarks
        for remark in remarks:
            m_match = re.search(r'^M\s(\d{1,3})', remark)
            if m_match and m_match.group(1) not in seen:
                logging.debug(f"Adding {remark} to {object_id}")
                m_sequence = m_match.group(1)
                common_name = f"M{m_sequence}"

                # Add Messier object
                desc = object_id_desc_dict.get(object_id, "")
                obj_rec = objects_db.get_catalog_object_by_sequence(catalog, sequence)
                objects_db.insert_catalog_object(obj_rec['object_id'], "M", m_sequence, desc)

                # Add name reference
                objects_db.insert_name(obj_rec['object_id'], common_name, "M")
                seen.add(m_sequence)
