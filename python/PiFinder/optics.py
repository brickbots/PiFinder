...
# Search window for the square-crop pipeline, where the FOV is ~10.2 deg for
# every supported camera. The full-frame window is derived from this per camera
# by SolveGeometry.solve_fov(), since how much wider the frame gets depends on
# the sensor's aspect ratio and how it is mounted.
CROP_FOV_ESTIMATE = 12.0
CROP_FOV_MAX_ERROR = 4.0
...
class OpticalCalibration:
    ...
    def __init__(self, fallback_fov: float, fallback_fov_max_error: float) -> None:
    ...
    @classmethod
    def for_geometry(cls, geometry: SolveGeometry) -> "OpticalCalibration":
        """Seed the pre-calibration search window from the square-crop one.

        The square crop has always solved at ~10.2 degrees, so scale that known
        window onto whatever frame we are actually solving instead of guessing a
        range wide enough for every camera.
        """
        estimate = geometry.solve_fov(CROP_FOV_ESTIMATE)
        upper = geometry.solve_fov(CROP_FOV_ESTIMATE + CROP_FOV_MAX_ERROR)
